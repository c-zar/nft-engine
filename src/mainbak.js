...
const startCreating = async () => {
  ...
  console.time("abstractedIndexes for loop");
  for (
  ...
  console.timeEnd("abstractedIndexes for loop");
  ...
  while (layerConfigIndex < layerConfigurations.length) {
    console.time("layersSetup");
    const layers = await layersSetup(
      layerConfigurations[layerConfigIndex].layersOrder
    );
    console.timeEnd("layersSetup");

    while (
      editionCount <= layerConfigurations[layerConfigIndex].growEditionSizeTo
    ) {
      console.time("createDna");
      let newDna = createDna(layers);
      console.timeEnd("createDna");


      if (isDnaUnique(dnaList, newDna)) {

        console.time("constructLayerToDna");
        let results = await constructLayerToDna(newDna, layers);
        console.timeEnd("constructLayerToDna");

        let loadedElements = [];

        console.time("loadedElements.push");
        await Promise.all(
          results.map(async (layer) => {
            let test = await loadLayerImg(layer)
            loadedElements.push(test);
          })
        )
        console.timeEnd("loadedElements.push");
        ...
        await Promise.all(loadedElements).then(async (renderObjectArray) => {
          ...
          console.time("addMetadata");
          addMetadata(newDna, abstractedIndexes[0]);
          console.timeEnd("addMetadata");

          console.time("saveMetaDataSingleFile");
          saveMetaDataSingleFile(abstractedIndexes[0]);
          console.timeEnd("saveMetaDataSingleFile");
